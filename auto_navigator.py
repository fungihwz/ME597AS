...
class Map():
    def __init__(self, map_name):
        ...
        # Store resolution and origin for coordinate transform
        self.resolution = self.map_df['resolution'][0]
        self.origin = self.map_df['origin'][0]
    ...
    def __open_map(self,map_name):
        ...
        mapname = "sync_classroom_map"
        file_path = f'/home/me597/lab4_ws/src/task_7/maps/{mapname}.yaml'
        f = open(file_path, 'r')
        #f = open(map_name + '.yaml', 'r')
        map_df = pd.json_normalize(yaml.safe_load(f))
        # Open the map image
        map_name = map_df.image[0]
        imgfile_path = f'/home/me597/lab4_ws/src/task_7/maps/{mapname}.pgm'
    ...
    def solve(self, sn, en):
        ...
        while len(self.q) > 0:
            ...
            for i in range(len(u.children)):
                ...
                if new_dist < self.dist[c.name]:  # Update if a shorter path is found
...
class Navigation(Node):
    ...
    def a_star_path_planner(self, start_pose, end_pose):
        ...
        map_resolution = mp.map.resolution
        map_origin = mp.map.origin

       
        # Convert PoseStamped to grid coordinates
        def to_grid_coords(pose):
            x_meters = pose.pose.position.x - map_origin[0]
            y_meters = pose.pose.position.y - map_origin[1]
            x_grid = int(x_meters / map_resolution)
            y_grid = int(y_meters / map_resolution)
            return f"{x_grid},{y_grid}"

        # Convert start and end poses
       
        start_grid = to_grid_coords(start_pose)
        end_grid = to_grid_coords(end_pose)
        self.get_logger().info(f"A* planner start: {start_grid}, end: {end_grid}")
        
        # Set up A* parameters with the converted grid coordinates
        mp.map_graph.root = start_grid
        mp.map_graph.end = end_grid

        if start_grid not in mp.map_graph.g or end_grid not in mp.map_graph.g:
            self.get_logger().error("Start or end node is outside map bounds or disconnected.")
            return None  # Exit if nodes are invalid
        ...
        path_as, dist_as = a_star.reconstruct_path(mp.map_graph.g[mp.map_graph.root], mp.map_graph.g[mp.map_graph.end])
        for coord in path_as:
            x, y = map(int, coord.split(','))
            pose = PoseStamped()
            pose.pose.position.x = x * map_resolution + map_origin[0]
            pose.pose.position.y = y * map_resolution + map_origin[1]
            path.poses.append(pose)

        # publish generated path to Rviz ui
    ...
    def get_path_idx(self, path, vehicle_pose):
        ...
        for i, pose in enumerate(path.poses):
            dx = vehicle_pose.pose.position.x - pose.pose.position.x
            dy = vehicle_pose.pose.position.y - pose.pose.position.y
    ...
    def path_follower(self, vehicle_pose, current_goal_pose):
        ...
        heading = 0.0 #np.arctan2(dy, dx)
        speed = 0.0
    
        self.get_logger().info(f"Path Follower: heading={heading:.2f}, speed={speed:.2f}")
    ...
    def run(self):
        ...
        while rclpy.ok():
            ...
            #print("goal_pose_received",goal_pose_received)
            if not self.goal_pose_received:
                self.get_logger().info("Waiting for a goal pose...")
                continue  # Skip if no new goal pose is received
            #if self.goal_pose and self.ttbot_pose:
            
            self.move_ttbot(0.0, 0.0)
            self.get_logger().info("goal_pose_received")
...
def main(args=None):
    ...
    # mp.inflate_map(kr,True)
    # mp.get_graph_from_map()
    # mp.map_graph.root = "70,125"
    # mp.map_graph.end = "140,180"
